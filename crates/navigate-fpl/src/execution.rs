...
impl PlanExecution {
    ...
    /// waypoint with an onward leg sequences within
    /// `max(capture_radius, min(DTA, inbound leg length))` of the fix
    /// (NAV-TT-003); fly-over and terminal waypoints sequence within the
    /// capture radius only (NAV-TT-004). A non-finite or negative
    /// groundspeed anticipates nothing. Once complete, every further
    /// call returns [`SequenceEvent::None`] without mutating anything.
    pub fn advance(&mut self, position: &GeodeticPosition, groundspeed_mps: f64) -> SequenceEvent {
        ...
        let anticipation = self.anticipation_distance(&leg, position, groundspeed_mps);
        let threshold = self.config.capture_radius_m.max(anticipation);
        if distance > threshold {
            return SequenceEvent::None;
        }
        let reason = if distance > self.config.capture_radius_m {
    ...
    /// The fly-by anticipation distance for the active leg, capped by
    /// the inbound leg length so a short leg cannot sequence before it
    /// begins; zero for fly-over fixes, the terminal fix, and any
    /// geometry too degenerate to define a course.
    fn anticipation_distance(
        &self,
        leg: &Leg<'_>,
        ...
        let inbound_start = leg.from.map_or(position, |from| &from.position);
        let inbound_length = distance_m(inbound_start, &leg.to.position);
        ...
        let inbound = initial_bearing_rad(inbound_start, &leg.to.position);
        ...
        turn_anticipation_m(radius, outbound - inbound).min(inbound_length)
    }
