import {selectedInputs,sequenceTrack} from './input-sequence.js';
...
const $=id=>document.getElementById(id);const map=new MapView($('map'));let regions=[],region,pack,mission,input,queryURL,loadedCamera,busy=false,media,baseMapLabel,activePipeline,mediaLoading=false;let inputFiles=[];
const pipelineSession=new PipelineSession();
window.addEventListener('pagehide',()=>pipelineSession.close());
...
async function loadMap(cam){map.setMinimumClearance(+$('agl').value);if(map.preview&&map.pack_id===pack.pack_id)await map.setCalibration(cam);else await map.load(pack,cam);loadedCamera=cam;$('map-empty').hidden=true;enable()}
async function openPrior(){await loadMap(camera());await map.setPose({position_enu_m:localPosition(pack,+$('latitude').value,+$('longitude').value,800),eye_to_enu_xyzw:[0,0,0,1]});baseMapLabel=$('map-label').textContent='REFERENCE AREA';$('attribution').textContent=pack.attribution;enable()}
...
function clearObservation(message){
  mission=null;$('export-track').disabled=true;$('missions').value='';$('frames').replaceChildren();$('hypotheses').replaceChildren();
...
$('locate').onclick=async()=>{const selectedPack=pack,selectedPrior=priorValues(),cam=camera();let pipeline;busy=true;clearObservation('Preparing this observation.');enable();try{
  ...
  await loadMap(cam);const loading=pipelineSession.acquire(selectedPack,cam,matchingOptions($('matching-quality').value),status);activePipeline=pipelineSession.current;enable();pipeline=await loading;
  const start=media.type==='video'?+$('frame-time').value:0,times=media.type==='image'?inputFiles.map(()=>0):[start];const sourceAspect=media.type==='image'?media.source.width/media.source.height:null;
  ...
  const frames=[],blobs=[];
  for(let i=0;i<times.length;i++){if(sourceAspect!==null){media.close();media=await openInput(inputFiles[i],$('video'));if(Math.abs(media.source.width/media.source.height-sourceAspect)>1e-6)throw Error('Sequence images must have the same aspect ratio. Process this image separately.')}clearObservation(`Processing frame ${i+1}/${times.length}. No estimate for this frame.`);const observation=await frameAt(media,times[i],cam);if(queryURL)URL.revokeObjectURL(queryURL);queryURL=URL.createObjectURL(observation.blob);$('query').src=queryURL;$('query-empty').hidden=true;const result=await pipeline.estimate(observation,selectedPrior,i,s=>status(`Frame ${i+1}/${times.length} · ${s}`));result.input_name=sourceAspect!==null?inputFiles[i].name:input.name;frames.push(result);blobs.push(observation.blob);await showHypothesis(result,hypotheses(result)[0]);$('details').textContent=JSON.stringify({pack_id:selectedPack.pack_id,camera:cam,observation:result},null,2)}
  mission=await storage.saveLocalMission({camera:cam,prior:selectedPrior,frames,input:{name:input.name,names:inputFiles.map(f=>f.name),size:inputFiles.reduce((n,f)=>n+f.size,0),processing:'browser-local',calibration:'assumed full-width 4:3 sensor crop; not independently calibrated'}},selectedPack,blobs);await refreshMissions(mission.id);await showMission(mission);
...
async function showMission(value){mission=value;const saved=await storage.get('packs',value.pack_id);if(!saved||!await storage.verifyPack(saved))throw Error('Saved observation requires a missing package. Download its region.');pack=saved;region=regions.find(r=>r.pack_id===pack.pack_id);if(!region)throw Error('Saved region catalog is missing');$('region').value=region.id;$('overview').src=assetUrl(region.thumbnail);
  ...
  $('frames').replaceChildren(...mission.view.frames.map((f,i)=>new Option(`${f.timing_scope==='still image'?(f.input_name||'Image '+(i+1)):(f.capture_time_ns/1e9).toFixed(2)+' s'} · ${frameLabel(f)}`,i)));$('frames').disabled=false;status(missionSummary(mission.view.frames));$('export-track').disabled=false;await showFrame(0)}
