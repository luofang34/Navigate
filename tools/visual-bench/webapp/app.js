...
const $=id=>document.getElementById(id);const map=new MapView($('map'));let regions=[],region,pack,mission,input,queryURL,loadedCamera,busy=false,media,baseMapLabel,activePipeline,mediaLoading=false,mediaMissionId=null,reviewContext=null,frameReview=null;let inputFiles=[];
...
$('frame-time').onchange=()=>{if(reviewContext&&media?.type==='video'){$('video').currentTime=+$('frame-time').value;return} $('frame-time-label').textContent=`${(+$('frame-time').value).toFixed(2)} / ${media.duration.toFixed(2)} s`;previewInput().catch(error)};
...
$('fetch-coverage').onclick=async()=>{busy=true;enable();$('fetch-coverage').disabled=true;try{await prepareOfflineStorage();
...
$('map').addEventListener('viewchange',({detail})=>{if(detail==='pose')return;if(detail==='overview')baseMapLabel='TRACK OVERVIEW';$('map-label').textContent=detail==='reset'||detail==='overview'?baseMapLabel:detail==='globe'?'GLOBE OVERVIEW · DISPLAY CONTEXT':'FREE CAMERA · RESET TO RETURN'});
