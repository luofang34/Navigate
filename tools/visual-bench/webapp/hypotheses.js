export function hypotheses(frame) {
  if (frame.candidate_hypotheses) return frame.candidate_hypotheses.filter(h => h.accepted || h.tracking_supported || h.scene_supported);
  return frame.accepted ? [frame] : [];
}
export function frameLabel(frame) {
  if(frame.decision==='scene_registration')return 'Conditional scene path';
