...
export class MapView{
  constructor(canvas){this.canvas=canvas;this.preview=null;this.pose=null;this.base=null;this.pending=false;this.again=false;this.drag=null;this.idle=Promise.resolve();this.frames=0;this.minimumClearance=20;
  ...
  async draw(){if(this.loading)return;if(this.pending){this.again=true;return this.idle}if(!this.preview||!this.pose)return;this.pending=true;let complete;this.idle=new Promise(resolve=>complete=resolve);
    try{let settle=2;do{this.again=false;await frameOpportunity();const clearance=constrainCamera(this.pack,this.pose,(lat,lon)=>this.preview.terrain_elevation_cached(lat,lon),this.minimumClearance);this.pose=clearance.pose;this.cameraAdjusted||=clearance.lift>.01;clearance.adjusted=this.cameraAdjusted;this.canvas.dataset.clearance=JSON.stringify(clearance);this.canvas.dispatchEvent(new CustomEvent('clearance',{detail:clearance}));if(this.sizeCanvas())this.preview.resize_display(JSON.stringify(this.displayCamera()));const start=performance.now();this.preview.present(JSON.stringify(this.pose));this.frames++;this.canvas.dataset.renderMs=(performance.now()-start).toFixed(2);this.canvas.dataset.frames=this.frames;this.canvas.dataset.presentation='wgpu-direct';this.canvas.dataset.projection='globe';}while(this.again||settle-->0)}finally{this.pending=false;complete()}
  }
