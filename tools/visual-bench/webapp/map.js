...
export class MapView{
  constructor(canvas){this.canvas=canvas;this.preview=null;this.pose=null;this.base=null;this.pending=false;this.again=false;this.drag=null;this.idle=Promise.resolve();this.frames=0;this.minimumClearance=20;this.constrain=true;this.baseConstrain=true;
    ...
    canvas.addEventListener('pointermove',e=>{if(!this.drag||!this.pose)return;const dx=e.clientX-this.drag[0],dy=e.clientY-this.drag[1];const look=this.drag[2];this.drag=[e.clientX,e.clientY,look];if(look||this.height()>50000)this.rotate(dx*.003,dy*.003);else this.pan(-dx,dy);this.draw().catch(console.error)});
  ...
  async setPose(pose,{constrain=false,immediate=false}={}){this.constrain=constrain;this.baseConstrain=constrain;this.cameraAdjusted=false;const globePose=toGlobePose(this.pack,pose);this.base=structuredClone(globePose);this.pose=structuredClone(globePose);await this.draw({immediate});this.canvas.dispatchEvent(new CustomEvent('viewchange',{detail:'pose'}))}
  changed(){this.constrain=true;this.cameraAdjusted=false;this.canvas.dispatchEvent(new CustomEvent('viewchange',{detail:'free'}))}
  async globe(){this.constrain=true;this.orbit=true;this.cameraAdjusted=false;this.pose={position_enu_m:[0,0,15_000_000],eye_to_enu_xyzw:[0,0,0,1]};await this.draw();this.canvas.dispatchEvent(new CustomEvent('viewchange',{detail:'globe'}))}
  async reset(){if(this.base){this.constrain=this.baseConstrain;this.cameraAdjusted=false;this.orbit=false;this.canvas.dispatchEvent(new CustomEvent('viewchange',{detail:'reset'}));this.pose=structuredClone(this.base);await this.draw()}}
  ...
  async draw({immediate=false}={}){if(this.loading)return;if(this.pending){this.again=true;if(immediate)this.wakeFrame?.();return this.idle}if(!this.preview||!this.pose)return;this.pending=true;let complete;this.idle=new Promise(resolve=>complete=resolve);
    try{let settle=2,first=true;do{this.again=false;if(!first||!immediate)await frameOpportunity(this);first=false;const clearance=constrainCamera(this.pack,this.pose,(lat,lon)=>this.preview.terrain_elevation_cached(lat,lon),this.minimumClearance);if(this.constrain){this.pose=clearance.pose;this.cameraAdjusted||=clearance.lift>.01}else{clearance.altitude-=clearance.lift;if(clearance.agl!==null)clearance.agl-=clearance.lift;clearance.lift=0;clearance.pose=structuredClone(this.pose)}clearance.adjusted=this.cameraAdjusted;clearance.constrained=this.constrain;this.canvas.dataset.clearance=JSON.stringify(clearance);this.canvas.dispatchEvent(new CustomEvent('clearance',{detail:clearance}));if(this.sizeCanvas())this.preview.resize_display(JSON.stringify(this.displayCamera()));const start=performance.now();this.preview.present(JSON.stringify(this.pose));this.frames++;this.canvas.dataset.renderMs=(performance.now()-start).toFixed(2);this.canvas.dataset.frames=this.frames;this.canvas.dataset.presentation='wgpu-direct';this.canvas.dataset.projection='globe';this.canvas.dispatchEvent(new CustomEvent('render',{detail:{pose:structuredClone(this.pose),camera:this.displayCamera()}}));}while(this.again||settle-->0)}finally{this.pending=false;complete()}
  }
...
function frameOpportunity(map){return new Promise(resolve=>{let frame,timer,finished=false;const done=()=>{if(finished)return;finished=true;cancelAnimationFrame(frame);clearTimeout(timer);map.wakeFrame=null;resolve()};map.wakeFrame=done;frame=requestAnimationFrame(done);timer=setTimeout(done,100)})}
