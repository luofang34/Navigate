...
try {
  const pack=await(await fetch('/api/offline-plan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({region_id:new URLSearchParams(location.search).get('region')||'new-jersey'})})).json();
  ...
  for(const mode of ['fast','balanced']) {
    ...
    try {
      ...
      report[mode].has_geometric_hypothesis=result.candidate_hypotheses.some(h=>h.accepted);
      document.getElementById('result').textContent=JSON.stringify(report,null,2);
    } finally {pipeline.close();}
  }
  bitmap.close();if(!report.balanced.has_geometric_hypothesis)throw Error('Balanced mode produced no geometric hypothesis');document.title='PASS matching detail comparison';
